    ...
    public function resendOtp(Request $request)
{
    ...
    try {
        ...
        // // ✅ 2. Generate a new OTP (optional if you’re using Twilio Verify)
        // $otp = rand(1000, 9999);
        // $user->update(['otp' => $otp]);

        // ✅ 3. Call Twilio to send OTP
        $verification = $this->twilio->sendOtp($user->phone);

        // ✅ 4. Handle Twilio response
        if ($verification && isset($verification->status) && $verification->status === 'pending') {
            return response()->json([
                'status'  => true,
        ...
        // ✅ Handle Twilio REST API exceptions (e.g., invalid number)
        return response()->json([
            'status'  => 'error',
            'message' => 'Twilio error: ' . $e->getMessage(),
        ], 400);
