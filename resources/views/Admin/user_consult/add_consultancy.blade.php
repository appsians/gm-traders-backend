...
<div class="page-content">
    <div class="d-flex justify-content-between align-items-center flex-wrap grid-margin mb-4">
        <div>
            <h4 class="mb-3 mb-md-0"> Consultancy Management</h4>
            <nav class="page-breadcrumb">
                <ol class="breadcrumb">
                  ...
                  <div class="d-flex align-items-center flex-wrap text-nowrap" id="addConsultancyBtn">
    ...
    <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title mb-4">All Consultancy Topics</h6>
...
<div class="modal fade" id="editConsultancyModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">

            <div class="modal-header">
                <h5 class="modal-title">Edit Consultancy</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            ...
            <form id="editConsultancyForm">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label">Consultancy</label>
                        <input type="text" name="consultancy" id="edit_consultancy" class="form-control" required>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">Sub Consultancy</label>
                        <input type="text" name="sub_consultancy" id="edit_sub_consultancy" class="form-control" required>
                    </div>
                </div>
...
$(document).ready(function() {
    ...
    var table = $('#consultanciesTable').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "{{ route('add.data') }}", // consultData route
            type: "GET",
        },
        columns: [
            {
                    ...
                    return `
                        <div class="action-buttons">
                            ...
                            <button class="btn btn-sm btn-info edit-btn"
                                ...
                                data-sub_consultancy="${row.sub_consultancy}"
                                title="Edit">
        ...
        language: {
            ...
            emptyTable: "No consultancy found",
            zeroRecords: "No matching consultancy found"
        }
    });
    ...
    $(document).on('click', '#bulkDeleteBtn', function() {
        ...
        Swal.fire({
            ...
            text: 'Are you sure you want to delete ' + selectedIds.size + ' selected consultancy topic(s)? This action cannot be undone.',
            icon: 'warning',
                    ...
                    success: function(response) {
                        if (response.status === true || response.status === 'success') {
                            Swal.fire({
                                icon: 'success',
                                title: 'Deleted',
                                text: response.message || 'Selected consultancy topics deleted successfully'
                            });
                            selectedIds.clear();
                            $('#selectAll').prop('checked', false);
                            updateBulkActions();
...
$(document).on('click', '.edit-btn', function () {
    ...
    $('#edit_consultancy').val($(this).data('consultancy'));
    $('#edit_sub_consultancy').val($(this).data('sub_consultancy'));

    $('#editConsultancyModal').modal('show');
});

// Submit Edit Form
$(document).on('submit', '#editConsultancyForm', function (e) {
    e.preventDefault();

    $.ajax({
        url: "{{ route('consult.update') }}",
        type: "POST",
        data: $(this).serialize(),
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
